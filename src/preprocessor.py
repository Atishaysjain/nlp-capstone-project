...
class EncoderWithChunks:
    ...
    def __init__(
        ...
        if self.max_tokens > self.tokenizer.model_max_length:
            raise ValueError(
                f"{model_name} only supports up to {self.tokenizer.model_max_length} tokens."
            )
    ...
    def _split_into_chunks(self, text: str) -> List[List[int]]:
        ...
        chunks = []
        for start in range(0, len(tokens), self.stride):   # uses new stride
    ...
    def encode(self, text: str) -> Union[np.ndarray, List[np.ndarray]]:
        ...
        for chunk_ids in self._split_into_chunks(text):
            ...
            if self.framework == "pt":
                ...
                hidden = outputs.last_hidden_state  # identical key

            if self.pooling == "cls":
                emb = hidden[:, 0, :]  # (1, hidden_dim)
            else:  # mean pooling over valid (non-pad) tokens
